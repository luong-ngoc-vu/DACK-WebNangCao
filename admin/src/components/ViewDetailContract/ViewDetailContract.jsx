...
import { Button, Col, Icon, Row, Select, Tabs, Tag, Typography, Modal } from 'antd';
...
class ViewDetailContract extends Component {
	...
	render() {
		const { item, dataStudent, dataTeacher, messages } = this.state;

		const { isLogin, idStudent, idTeacher } = this.props;
		if (isLogin === false) {
		...
		return (
			<div>
				<div>
						...
						<Col span={20}>
							<div className="header-detail-contract">
								...
								{item.status === 0 ? (
									...
									<Tag color="orange" style={{ height: 25 }}>
										Đã kết thúc
									</Tag>
								) : item.status === -1 ? (
									...
									<Tag color="yellow" style={{ height: 25 }}>
										Khiếu nại
									</Tag>
								)}
							...
							>
								<Col span={1} />
								<Col span={4}>
									<img src={dataTeacher.image} className="avatar-user" />
								</Col>
								<Col span={19}>
									<div className="info-user">
										<div className="info-tutor" style={{ width: '100%', position: 'relative' }}>
											...
											>
												...
												<span style={{ fontWeight: 500 }}>
													&ensp;
													{dataTeacher.address}, {dataTeacher.wardName},{' '}
													{dataTeacher.districtName}, {dataTeacher.provinceName}
												</span>
											</Text>
							...
							<Row>
								<Col span={1} />
								<Col span={4}>
									<img className="avatar-user" src={dataStudent.image} alt="" />
								</Col>
								<Col span={19}>
									<div className="info-hirer">
										...
										<Text style={{ marginBottom: 5 }}>
											<Icon type="book" />
											&ensp;Môn thuê: <Tag style={{ fontSize: 16 }}>{item.skills}</Tag>
										</Text>
										<Text style={{ marginBottom: 5 }}>
										...
										item.status === -2 && (
											<Text type="danger" style={{ marginBottom: 5, textAlign: 'left' }}>
												...
												<span style={{ fontWeight: 500 }}>{item.noiDungKhieuNaiHS}</span>
											</Text>
										)}
									</div>
								</Col>
					...
					<Modal
						style={{ top: 20 }}
						title="Xem cuộc trò chuyện"
						visible={this.state.visible}
						onOk={this.handleOk}
						onCancel={this.handleCancel}
					>
						<Messages mailTeacher={dataTeacher.email} mailStudent={dataStudent.email} />
					</Modal>
				</div>
			</div>
		);
