...
import { Button, Checkbox, DatePicker, Form, Input, InputNumber, Radio, Row, Select, Typography, Col } from 'antd';
	...
	render() {
		...
		return (
			<Form layout="vertical" onSubmit={this.handleSubmit} style={{ maxHeight: '100%' }}>
				<Row gutter={15}>
					<Col span={12}>
						<Form.Item label="Tên giáo viên">
							<Input style={{ width: '100%' }} value={st.nameTeacher} readOnly />
						</Form.Item>
					</Col>
					<Col span={12}>
						<Form.Item label="Tên học sinh">
							<Input style={{ width: '100%' }} value={st.nameStudent} readOnly />
						</Form.Item>
					</Col>
				</Row>
				<Form.Item label="Giới tính">
					<Input style={{ width: '100%' }} value={st.genderStudent} readOnly />
				</Form.Item>
				<Row gutter={15}>
					<Col span={12} />
				</Row>
				...
				<Form.Item label="Lịch học trong tuần">
					{getFieldDecorator('schedule', {
						...
						<div>
							...
							<Row>
								<Checkbox.Group options={timeDay} onChange={this.onChangeTimeDay} />
							</Row>
						</div>
					)}
				</Form.Item>
				...
				<Form.Item>
					<Checkbox checked={true}>Bạn đồng ý với những điều khoản của Bmentor</Checkbox>
				</Form.Item>
				<div>
				...
				{st.curMoneyStudent < this.state.totalMoneyContract && (
					<div style={{ justifyContent: 'center' }}>
						<strong>
							Số tiền hiện có ({st.curMoneyStudent}) không đủ để thuê gia sư này. Bạn phải có tối thiểu số
							tiền là ({this.state.totalMoneyContract})
						</strong>
					</div>
				)}
				<Form.Item
						...
						<Button
							...
							onClick={(event) => {
								st.addNewContract(
									...
									st.nameTeacher,
									st.genderStudent,
									st.moneyTeacherPerHour,
									this.state.totalMoneyContract,
